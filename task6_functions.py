...
profit = calculate_total_sales(video_game_sales[1])    
print(profit)
...
print(filter_by_genre(video_game_sales, "Racing"))


def get_summary(game):
        return f"{game[1]} ({game[3]}) - {game[4]} - ${game[9]}M" 
    
print(get_summary(video_game_sales[0]))

#In task 6, section c, instructions asked for a loop. Review only mentioned using a single game as an argument. 
#Due to this discrepancy, I included this loop for the entire dataset as a comment, as well as the previous line of code for just one game as an argument.
